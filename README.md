...
6. **При статусе `paid`** засчитайте транзакцию и выполните заказ

### Примерный код workflow (PHP)
```php
<?php
// 1. Создание заказа
$internalUUID = 'order-' . uniqid();
$orderData = [
    'amount' => '1500.00',
    'currency' => 'RUB',
...
$transactionResult = createTransaction($publicKey, $secretKey, $orderData);

if ($transactionResult['success']) {
    // 3. Сохранение в БД
    saveTransaction([
        'internal_uuid' => $internalUUID,
        'transaction_uuid' => $transactionResult['transaction_uuid'],
        'payment_link' => $transactionResult['payment_link'],
        'status' => 'pending'
    ]);
    
    // 4. Показать клиенту ссылку
    header("Location: " . $transactionResult['payment_link']);
    exit;
} else {
    // Обработка ошибки
    error_log("Transaction creation failed: " . $transactionResult['error']);
    showErrorPage("Ошибка создания платежа");
}
?>
```
...
- **Таймаут**: Учитывайте время ожидания оплаты (обычно 5-10 минут)
